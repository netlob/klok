var letters = {
  dutch:   "HETKISAVIJF TIENATZVOOR OVERMEKWART HALFSPMOVER VOORTHGEENS TWEEAMCDRIE VIERVIJFZES ZEVENONEGEN ACHTTIENELF TWAALFPMUUR",
  english: "ITLISASAMPM ACQUARTERDC TWENTYFIVEX HALFSTENFTO PASTERUNINE ONESIXTHREE FOURFIVETWO EIGHTELEVEN SEVENTWELVE TENSEOCLOCK"
}

for (var i = 0; i < letters[language].length; i++) {
  if(letters[language][i] == " ") continue
  document.querySelector(".grid").innerHTML += `<span id="${i}" class="inactive">${letters[language][i]}</span>` 
}
...
const formatTime = async(i) => {
  var res = minutes[language].base,
      d = new Date(i),
      h = d.getHours() > 11 ? d.getHours() - 12 : d.getHours(),
      m = d.getMinutes(),
      m = parseInt((m % 5) >= 2.5 ? parseInt(m / 5) * 5 + 5 : parseInt(m / 5) * 5),
      res = [...res,...hours[language][m > minutes[language].next ? h + 1 : h],...minutes[language][m]];
  return res;
}
